...
//Steffen: I thought this was the most obvious breakpoint so I chose to place a try/catch here
function initialize() {
   try { myGrid = new Grid();
    moves = 0;
    winner = 0;
    ...
    display.error("Game could not load: ", error)
}
}
...
function endGame(who) {
    ...
    //added window per instructions, thank you message appended to html doc
    if (confirm("Play again?")) {
        setTimeout(restartGame, 800)
    } else{
        document.getElementById("dev").append("Thank you for playing! \nDeveloped by Steffen Turner");
    };
}
